...
def test_risk_spread_block(tmp_path):
    ...
    decision = rm.evaluate("BTCUSDT", signal, last_price=100.0, open_positions=0, config=_config(), spread=0.01)
    assert not decision.allowed


def test_risk_max_trades(tmp_path):
    store = SQLiteStore(str(tmp_path / "r2.db"))
    store.add_trade("BTCUSDT", "BUY", 1.0, 100.0, "paper", "paper", None)
    rm = RiskManager(store)
    signal = Signal(side="BUY", reason="test", stop_loss=99.0)
    decision = rm.evaluate("BTCUSDT", signal, last_price=100.0, open_positions=0, config=_config(), spread=0.0)
    assert not decision.allowed
...
def test_risk_circuit_breaker(tmp_path):
    ...
    store.add_trade("BTCUSDT", "BUY", 1.0, 100.0, "paper", "paper", None)
    store.add_trade("BTCUSDT", "SELL", 1.0, 90.0, "paper", "paper", None)
    rm = RiskManager(store)
    signal = Signal(side="BUY", reason="test", stop_loss=99.0)
    decision = rm.evaluate("BTCUSDT", signal, last_price=100.0, open_positions=0, config=_config(), spread=0.0)
